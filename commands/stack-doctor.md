...
- [ ] `~/.claude/commands/` contains at least `prepare-claude-design.md`, `generate-design-request.md`, `finalize-build-docs.md`, `notion-setup.md`, and `stack-doctor.md`, symlinked.
...
- [ ] `AGENTS.md` exists, is committed (not gitignored), and opens as the **execution contract**: the "You are the EXECUTOR" preamble, the read-order (`Product Specification.md` → `Implementation Plan.md` → this file), and the Non-negotiables (conflict order, Fidelity gate, reuse-not-rebuild, checkbox protocol). **Pointer-style is valid**: skills listed as a routed index (description + `.skills-source/...` path) pass, PROVIDED `.skills-source/` is present in the repo so the pointers resolve — pointer-style with a missing `.skills-source/` is a ✗ (the executor's rules point at nothing; fix: `npm run sync-skills`). A headerless AGENTS.md (no contract preamble) is a ✗ regardless of style.
...
- [ ] **Design mode** — read the `Design mode` line in `design/CLAUDE_DESIGN_PROMPT.md` and report it (`Claude Design` / `Prompt only`). If it is missing, infer it from the evidence (prototypes or `design/design-release.json` → Claude Design; neither, with per-screen specs in the Design Reference → Prompt only) and report it as inferred. The remaining design checks depend on it.
- [ ] `design/` has the subfolders its mode requires — Claude Design: `prototypes/`, `system/`, `planning/`, `handoff/`; Prompt only: `system/`, `planning/`, `handoff/`, and **no** `prototypes/`, `design-release.json`, or `design-sync.lock.json` (their presence in a prompt-only project is a ✗: the mode is mixed). Report which contain files.
- [ ] Everything under `design/` is **committed** — flag any untracked file (uncommitted contract = the Fidelity QA check has no frozen target). Flag `design/` being gitignored as a critical error. In Claude Design mode, screen prototypes follow the `*.dc.html` / `logo--*.html` convention.
- [ ] Sanity (Claude Design): `design/prototypes/` non-empty while `design/system/` is empty is a warning (Product Specification §1 will be inferred, not exported).
- [ ] `design/handoff/[PROJECT] Design Reference.md` and `design/handoff/[PROJECT] Design Handoff Plan.md` both exist and link to each other. Every screen in `design/planning/screen-inventory.md` maps to its source — a prototype file (Claude Design) or a Design Reference section (Prompt only). Report missing mappings.
- [ ] Root `Product Specification.md` and `Implementation Plan.md` exist and link to each other. Report missing pairs.
- Fix for stale sync: `npm run sync-skills`. Fix for missing build docs: `/sync-build-docs [ProjectName]` or `/finalize-build-docs [ProjectName]` (Claude Design), or `/prepare-claude-design [ProjectName] --prompt-only` (Prompt only). Fix for design-blocked tasks: `/generate-design-request [ProjectName]`. Fix for old-style CLAUDE.md / missing workflows: copy the current blocks from the boilerplate (Phase 4 of task_agent_stack.md).
