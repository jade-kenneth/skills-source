...
Run `npm run design:validate-final` first. Compared with `design:validate`, this
mode *additionally* accepts an unchanged, already synchronized final release; it
does not reject a final release that has not been synchronized yet, so passing it
is not by itself proof of synchronization. Require `design/design-release.json`
with `"status": "final"`. Required MVP scope must have no `stillInDesign`,
...
3. **App mapping** — ask me which boilerplate app becomes which product surface, including which stable boilerplate apps back each product surface and any full-app removals (e.g. `apps/app-web` hosts BOTH the consumer app and `/admin`; `apps/app-mobile` is removed with its push/auth responsibilities re-covered via Capacitor). Preserve the stable app paths and Nx identifiers; do not propose product-named folder renames. Do not assume a mapping.
...
**Prototypes (`design/prototypes/`)** are the **pixel-exact and observable-outcome-exact contract**, not loose inspiration. For every screen, require exactly one `data-prototype-surface` and one `data-app-root`. Extract every font-family/size/weight/line-height, hex color, spacing value, border/shadow/radius/rotation, element and its order, state, interaction outcome, and animation from inside the app root. Preserve visible copy verbatim. A prototype handler shows what the user must observe; its local state, mock records, manual checks, fake delay, storage, or network mechanism is never the required production implementation. Do not treat `data-preview-shell`, `data-handoff="presentation-only"`, device frames, desktop centering canvases, browser chrome, measurement labels, or design-tool annotations as production UI.
...
App mapping: as confirmed in **Runtime inputs #3**. **Preserve the stable architecture paths and Nx identifiers**: `apps/app-web`, `apps/app-api`, `apps/app-mobile`, and `packages/shared-constants`. They are upstream integration identifiers, not customer-facing names. Apply `$ARGUMENTS` through product identity settings such as display names, root package metadata, URL/app slugs, mobile bundle identifiers, database names, domains, environment configuration, branding, and copy. Do not add product-named app-folder, workspace-package, or Nx-project renames to the plan. Full-app removals must list their ripples (workspace config, codegen paths, CI, Docker, responsibilities, and root docs) as explicit tasks. Treat backend/plumbing boilerplate (GraphQL client+server, TanStack Query, auth, authz, S3, CI) as **reuse-not-rebuild**: for every feature, find the existing primitive and extend it in the established pattern; never re-implement auth, authz, or the client/codegen pipeline. Mark foundation tasks as already-provided rather than to-build.
...
3. **Every primary screen** (client) — one subsection per `screen--*.html` file, treated as a **platform-aware fidelity spec**, not a loose description. For each screen include: (a) the **source prototype file, declared surface, and `data-app-root` boundary** it comes from, so an engineer can inspect the real contract without copying preview chrome; (b) exact layout with real values (font-family/size/weight/line-height, hex colors, px spacing/gaps, borders, shadows, radii, rotations — lifted from the prototype, not approximated); (c) exact copy, verbatim; (d) **every** state, interaction outcome, and animation (nothing "minor" omitted — hover/press/active states, transitions, empty/loading/error states, toggles, moderation blocks, etc.); and (e) a closing **"Production mapping (Phase N)"** table that names its architecture source, classifies state ownership, identifies the configured read/write operations and typed wrappers, traces the applicable server/application/persistence path, identifies cache keys/invalidation or optimistic rollback, separates configured client feedback validation from authoritative server validation/authz, and names the loading/error/offline contract. Use the current boilerplate's GraphQL/codegen/TanStack Query and resolver/service/repository paths when present; otherwise use the approved configured equivalents. Explicitly list prototype-only local/mock/manual mechanisms that must not be carried over. End each screen with a one-line **fidelity check**: "matches [prototype file] exactly — fonts, layout, spacing, colors, states, functions."
...
- [ ] **Routing** — the screen is registered at its declared `data-route` with the declared params, navigation container, and presentation; it is reachable from every declared entry point; its guard redirects and post-sign-in return behave as declared; and its loading, not-found, invalid-param, and permission-denied results render the designed states. Back, cancel, and dismiss land on the declared screen, including Android hardware back, iOS swipe-back, and the browser back button.
- [ ] **Control bindings** — every `data-action-id` in the prototype has a production handler producing its declared `data-action-result`; no control is left unbound, dead, or silently no-op; destructive actions show their named confirmation surface; and controls hidden, disabled, or retargeted per role behave that way for each role.
...
- [ ] **Architecture mapping** — every data-backed state and action follows the documented production owner and end-to-end path; server records are not mirrored as authoritative local state, forms use the established schema stack, API validation/authz is enforced, and query cache updates or rollback keep all consumers consistent.
- [ ] **No prototype runtime leakage** — no mock arrays, fake persistence, manual-only validation, hard-coded permissions, fake delays, or prototype network handlers remain in the production path.
...
- Every stand-in carries a `QA-BYPASS(Phase N)` marker comment — including files placed under `qa-fixtures/` — and never ships beyond QA.
- The screen stays `[~]` while any `🔁` row remains. When the blocking phase completes, the executor removes the stand-in, confirms **both** zero `QA-BYPASS` marker hits and zero `qa-fixtures/` references for that screen, and re-runs exactly the `🔁` rows against the real path; only then may the screen be `[x]`. "No prototype runtime leakage" is always in the re-verify set whenever a stand-in was used.
