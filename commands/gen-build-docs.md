---
description: Generate paired [PROJECT]Reference.md + [PROJECT] Task Plan.md build docs from design prototypes in design/prototypes/
...
3. **App mapping** — ask me which boilerplate app becomes which product surface, including renames and full-app removals (e.g. `apps/app-admin` → `[project]-web` hosting BOTH the consumer app and `/admin`; `apps/app-mobile` removed with its push/auth needs re-covered via Capacitor). Do not assume a mapping.
...
- **Design prototypes: read every file in `design/prototypes/`** — these are the **pixel-exact, behavior-exact contract**, not loose inspiration. Files follow the naming convention `screen--<name>.html` (or `.dc.html`) for screens and `logo--<name>.html` for identity. Extract the _actual code_: every font-family/size/weight/line-height, every hex color, every px of padding/margin/gap, every border/shadow/radius/rotation, every element and its order, every state, interaction, handler, and animation. Do not summarize a screen into prose that drops details — if a value is in the prototype, it must survive into the docs verbatim.
- **Completeness rule:** every `screen--*.html` file in `design/prototypes/` MUST produce its own §3 (or §4) subsection in the Reference. Before writing, list all prototype files and map each to a planned section; if any file has no section, or any expected screen has no file, surface the gap to me explicitly instead of silently omitting it.

**Fidelity mandate (read first, applies to everything below):** the goal is a **total, faithful copy** of the provided prototype HTML — same fonts, same layout, same spacing, same colors, same micro-details, same functions/interactions. The implementation must look and behave identically to the prototype; the only thing that changes is that mock/hard-coded data becomes real data from the confirmed **MONGODB_URI**. Engineers should **port the prototype's markup + styles as the starting DOM and wire data into it**, NOT rebuild each screen from a written description. When in doubt, the prototype's literal code wins over any paraphrase.

- Any design-system or brand notes (e.g. `CLAUDE.md`, tokens files, `.skills-source/conventions/`) — extract fonts, exact color hex tokens, spacing, style "moves", voice.
- Any existing task/plan/PRD files — reuse their phase structure rather than inventing one.
- The logo / identity assets — treat the **logo prototype file** (`logo--*.html` and its chosen option) as the mark's source of truth; it is ported verbatim like any other screen.
...
0. **What it is** + product stack + an "API convention (read this first)" callout + a "⚠ build on the existing backend boilerplate, don't scaffold from scratch — but discard boilerplate UI and build the design system's UI exactly" directive + the confirmed **MONGODB_URI** connection + a "database is always seeded (login + display data)" note.
...
- If there's no design system yet, ask me for visual direction (vibe, audience, colors, type) before distilling.
- If `design/prototypes/` is empty or missing, stop and tell me — offer to work from the codebase / Figma / screenshots instead, but do not invent UI.
