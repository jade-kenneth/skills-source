...
- **Decorative art never ships as an image element** → background blobs, mascots, drifting shapes, glows, and pattern overlays go in `background-image` on an `aria-hidden` element; `<img>` / the framework image component is reserved for images that carry meaning. Search engines pick the result thumbnail from the page's own images (not `og:image`), favouring whatever appears early and repeats — and `alt=""` does not exclude it — so decorative art rendered as an image will hijack the search listing. Reinforce the intended image with a descriptive `alt` + `priority`, JSON-LD carrying an explicit `image`, and `max-image-preview: large` in root metadata. See `references/nextjs-performance-seo.md` § 2b. Search-Result Thumbnails.
