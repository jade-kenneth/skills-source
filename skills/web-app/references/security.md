...
- Prefer secure defaults for cookies, tokens, and third-party integrations.
...
| Server-only API keys are never exposed client-side | 🔴 | In Next.js, only `NEXT_PUBLIC_*` vars reach the browser — verify all others are server-only |
...
Every production deployment should include these HTTP headers:

| Header | Value | Purpose |
| --- | --- | --- |
| `Content-Security-Policy` | `default-src 'self'; script-src 'self' 'nonce-...'` (customized) | Prevents XSS by controlling script sources |
...
```tsx
// filepath: next.config.ts — security headers in Next.js
...
# In Next.js, only NEXT_PUBLIC_* should appear in client bundles
...
| **Secrets in client bundle** | Do `grep .next/static/` results contain non-`NEXT_PUBLIC_` env vars? | 🔴 |
