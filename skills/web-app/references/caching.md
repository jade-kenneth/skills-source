...
- Avoid raw `fetch`, `axios`, or manual `useEffect` + `useState` for server state unless no suitable tool is available.
...
- Do not reload the page after create, update, or delete flows when a targeted cache update or refetch can keep the UI in sync.
- Never invalidate the entire cache by default — invalidate only the affected entity detail, related lists, or directly impacted scopes.
...
- When building paginated admin lists, prefer a single reducer-backed page state object such as `{ page, pageSize, ...filters }` instead of separate pagination and filter states.
- Always debounce search inputs with `useDebounce` (300ms default). Do not use `useDeferredValue` as a substitute — it does not delay network requests.
- Reset `page` back to 1 whenever search terms or filters change.
- After deletes, filter changes, or shrinking search results, clamp `page` to the last valid page.
- Verify the backend contract before building admin search, filter, create, edit, or status flows.
...
- Invalidate only affected scopes — avoid global invalidation by default.
- Prefer direct cache updates only when the updated value is complete and deterministic.
- Prefer hybrid mutation handling: update detail cache directly, then invalidate related lists.
- Use optimistic updates only for deterministic, low-risk interactions. Always include rollback handling.
...
- Prefer client-side cache updates and targeted invalidation over full page reloads.
...
2. [TanStack Query Caching](#2-tanstack-query-caching)
   ...
   - [Hydration (SSR / App Router)](#29-hydration-ssr--app-router)
...
3. [Apollo Client Caching](#3-apollo-client-caching)
   ...
   - [SSR & Next.js App Router Integration](#310-ssr--nextjs-app-router-integration)
...
### 2.9 Hydration (SSR / App Router)

For Next.js App Router, prefetch on the server and hydrate to the client cache:

```tsx
// filepath: lib/providers/QueryProvider.tsx
'use client';
...
```tsx
// filepath: app/notes/[slug]/page.tsx (Server Component)
import { dehydrate, HydrationBoundary } from '@tanstack/react-query';
import { makeQueryClient } from '@/lib/queryClient';
...
export default async function NoteDetailPage({
  ...
  await queryClient.prefetchQuery({
    queryKey: noteKeys.detail(params.slug),
    queryFn: () => fetchNoteBySlug(params.slug),
  });

  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <NoteDetailClient slug={params.slug} />
    </HydrationBoundary>
  );
}
```
...
3. Render HTML + serialized state
                                    4. HydrationBoundary receives state
                                    5. Hydrate into client QueryClient
                                    6. useQuery() finds data in cache → no fetch
                                    7. Data is fresh → instant render
```
...
### 3.10 SSR & Next.js App Router Integration

#### Server-Side Data Fetching with Apollo

```tsx
// filepath: lib/apolloServerClient.ts
...
```tsx
// filepath: app/notes/page.tsx (Server Component)
import { getClient } from '@/lib/apolloServerClient';
import { GET_NOTES } from '@/graphql/queries/notes.gql';

export default async function NotesPage() {
  const { data } = await getClient().query({
    query: GET_NOTES,
    fetchPolicy: 'network-only', // Always fresh on server
  });
...
#### Apollo + Client Components (App Router)

```tsx
// filepath: lib/providers/ApolloProvider.tsx
'use client';
...
## Apollo Client — Project Conventions
...
| Rule                                                                                     | Why                                                              |
| ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| Use `useQuery(CART_QUERY)` / `useMutation(ADD_TO_CART_MUTATION)` with document constants | Keeps operations traceable and typed                             |
| Do not use generated hook wrappers (`useCartQuery`, `useAddToCartMutation`) in UI code   | Generated wrappers couple UI to codegen output                   |
| Import shared enums/types/scalars from `~/graphql/generated` only                        | Types and enums are stable; generated hooks are not              |
| Operation names must be unique and PascalCase                                            | Required by Apollo DevTools and codegen                          |
| Prefer Apollo hooks or `apolloClient` over raw `fetch`/`axios` for GraphQL data          | Consistent cache behavior; bypassing Apollo breaks normalization |

### Acceptance Criteria

Before merging any GraphQL-related change:

- New/changed operations exist as typed document constants in the appropriate domain file.
- UI code uses `useQuery` / `useMutation` with those constants.
