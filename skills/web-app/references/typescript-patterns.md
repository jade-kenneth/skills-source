...
- Prefer `unknown` at external boundaries (network responses, storage, env vars, user input, `JSON.parse`). Narrow with validators, type guards, or assertion functions.
- Standardize reusable helpers: `Prettify<T>`, `ValueOf<T>`, `NonEmptyArray<T>`, `Brand<T, Name>`.
...
- Prefer `satisfies` over broad `as` when checking object shapes while preserving inference.
- Prefer `as const` with literal unions when runtime enum behavior is not required.
- Use branded types for critical identifiers (`UserId`, `OrderId`, etc.).
...
- Avoid broad `as` except at validated boundaries, branding, or proven invariants.
- Do not use non-null assertions (`!`) to silence errors.
- Do not destructure object properties at the top level of a component or large function when those properties are also accessed via `object.property` elsewhere in the same scope. Prefer consistent `object.property` access; destructuring is acceptable inside small focused functions where all extracted values are used locally.
